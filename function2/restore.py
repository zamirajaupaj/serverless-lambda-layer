...
def restore_db(cursor, dbName, S3BucketName, KmsKeyId, Region, AccountId, FileName):
    try:
        cursor.execute("""
        IF EXISTS(select * from sys.databases where name='zamira')
        DROP DATABASE zamira
        """)
        sql = (
            "exec msdb.dbo.rds_restore_database " \
                f"@restore_db_name='{dbName}', "  \
                f"@s3_arn_to_restore_from='arn:aws:s3:::{S3BucketName}/{FileName}', " \
    ...
    try:
        ...
        if (conn) is None:
            ...
            if status[1] == 'SUCCESS' or status[1] == 'ERROR' or status[1] == 'CANCELLED'or status[1] == 'CANCEL_REQUESTED' or status[1] == 0:
                restore_db(cursor,  event['DbName'], event['S3BucketName'], event['KmsKeyId'],event['Region'], event['AccountId'], event['FileName'])
            else:
